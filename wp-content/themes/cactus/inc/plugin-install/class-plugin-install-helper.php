...
class Cactus_Plugin_Install_Helper {
	 ...
	 * @return string
	 */
	public function get_button_html( $slug,$init ='' ) {
		
		if( $init == '')
			$init = $slug;
			
		$button = '';
		$state  = $this->check_plugin_state( $slug,$init );
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				...
				case 'activate':
					if ( $slug == 'mailin' ) {
						...
						$plugin_link_suffix = $slug . '/' . $init . '.php';
					}
	 ...
	 * @return bool
	 */
	public function check_plugin_state( $slug, $init = '' ) {
		if( $init == '')
			$init = $slug;
			
		if ( file_exists( ABSPATH . 'wp-content/plugins/' . $slug . '/' . $init . '.php' ) || file_exists( ABSPATH . 'wp-content/plugins/' . $slug . '/index.php' ) ) {
			$needs = ( is_plugin_active( $slug . '/' . $init . '.php' ) || is_plugin_active( $slug . '/index.php' ) || is_plugin_active( $slug . '/sendinblue.php' ) ) ? 'deactivate' : 'activate';
