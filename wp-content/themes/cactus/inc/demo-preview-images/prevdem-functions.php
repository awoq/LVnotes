 ...
 * @return mixed
 */
function cactus_get_prevdem_img_src( $i = 0 ) {
	...
	if ( ! file_exists( $path . $img_name ) ) {
		...
		$i++;
		return cactus_get_prevdem_img_src( $i );
	}
 ...
 */
function cactus_the_post_thumbnail( $input ) {
	if ( empty( $input ) ) {
		$placeholder = cactus_get_prevdem_img_src();
		return '<img src="' . esc_url( $placeholder ) . '" alt="" class="attachment-cactus-blog size-cactus-blog wp-post-image">';
	}
	return $input;
}

add_filter( 'post_thumbnail_html', 'cactus_the_post_thumbnail' );
