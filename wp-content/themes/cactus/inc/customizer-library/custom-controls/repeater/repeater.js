...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend({
	...
	ready: function() {
		...
		// Init the control.
		if ( ! _.isUndefined( window.cactusControlLoader ) && _.isFunction( cactusControlLoader ) ) {
			cactusControlLoader( control );
		} else {
			control.initCactusControl();
		}
	},

	initCactusControl: function() {
		...
		// The current value set in Control Class (set in Cactus_Customize_Repeater_Control::to_json() function)
		var settingValue = this.params.value;

		control.container.find( '.cactus-controls-loading-spinner' ).hide();
	...
	setImageInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.cactus-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.cactus-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ) {
		...
		$targetDiv.find( '.cactus-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ) {
		...
		$targetDiv.find( '.cactus-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ) {
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Cactus_Customize_Repeater_Control::render_content() ).
