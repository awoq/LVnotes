...
add_action( 'customize_register', 'cactus_repeater_customize_register' );
function cactus_repeater_customize_register(){
/**
 * Repeater control
 */
class Cactus_Control_Repeater extends WP_Customize_Control {
	...
	/**
	 * The cactus_config we're using for this control
	 *
	 * @access public
	 * @var string
	 */
	public $cactus_config = 'global';
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'cactus' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'cactus' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function enqueue() {
				...
				if ( isset( $field['type'] ) ) {
					...
					switch ( $field['type'] ) {
						...
						case 'dropdown-pages':
							...
							wp_enqueue_style( 'cactus-select2', CUSTOMIZER_LIB_URI . '/assets/vendor/select2/mageewp.css', null );
							break;
						case 'iconpicker':
		...
		wp_enqueue_script( 'cactus-repeater', CUSTOMIZER_LIB_URI. '/custom-controls/repeater/repeater.js', array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
		wp_enqueue_style( 'cactus-repeater-css', CUSTOMIZER_LIB_URI . '/custom-controls/repeater/repeater.css', null );
	}
	...
	protected function render_content() {
		?>
		<div class="cactus-controls-loading-spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>
		<label>
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'cactus' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-header">
					...
					<span class="repeater-row-remove" style="cursor:pointer;"><?php esc_attr_e( 'Remove', 'cactus' ); ?></span>
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'cactus' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( 'textarea' === field.type ) { #>
								...
								<# } else if ( 'editor' === field.type ) { #>
									 <div class="customize-control-cactus-editor">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="cactus-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'cactus' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'cactus' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'cactus' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'cactus' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'cactus' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'cactus' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'cactus' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="cactus-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'cactus' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'cactus' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'cactus' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'cactus' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'cactus' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'cactus' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'cactus' ); ?></button>
				</div>
			</li>
		</script>
		<?php
