...
function cactus_customizer_library_options() {
	...
	$options['transparent_header'] = array(
			...
			'label'   => __( 'Make Header Transparent on Frontpage', 'cactus' ),
			'section' => $section,
			'type'    => 'checkbox',
			'default' => '1',
			'transport' => $transport,
...
<a href="%s" target="_blank">%s</a>', 'cactus' ),esc_url('https://fontawesome.com/v4.7.0/icons/'),esc_url('https://fontawesome.com/v4.7.0/icons/'))),
			...
			case "contact":
				...
				$options['google_map_api_'.$key] = array(
					...
					'type'    => 'text',
					'default' => '',
					'description' => sprintf(__('Enter a correct Google Map API Key and Google Map Address to display the google map. <a href="%s" target="_blank">Get API Key</a>', 'cactus' ),esc_url('https://developers.google.com/maps/documentation/javascript/get-api-key'))
				);
				...
				$options['receiver_email_'.$key] = array(
					'id' => 'receiver_email_'.$key,
					'label'   => __( 'Receiver Email', 'cactus' ),
					'section' => $section,
					'type'    => 'text',
					'default' => '',
					'transport' => 'postMessage',
					'description' => __('Receive email from contact form.', 'cactus' )
				);
				
				$plugin = 'vela-companion';
				...
				$description = __( 'Install & active plugin "Vela Companion" to enable the contact form & google map.', 'cactus' ). Cactus_Plugin_Install_Helper::instance()->get_button_html( $plugin );
				
				$options['notice_'.$key] = array(
					'id' => 'notice_'.$key,
					'label'   => __( 'Contact Form', 'cactus' ),
					'section' => $section,
					'type'    => 'content',
					'default' => '',
