...
<div class="cactus-section-content">
  ...
  <?php if ( $container == 'cactus-container-fullwidth' ){?> <div  class="container"> <?php } ?>
    ...
    <?php if ( $container == 'cactus-container-fullwidth' ){?> </div> <?php } ?> 
	...
	$items .= '<a href="'.esc_url($large_image).'" rel="prettyPhoto[pp_gal]"><i class="fa fa-search-plus"></i></a>'.(($link!='')?'<a href="'.esc_url($link).'" ><i class="fa fa-link"></i></a>':'').'</div>
            </div>
			</div>
          </div>
        </div>
    ...
    <nav class="cactus-portfolio-filter">
	  ...
	  <?php if( !empty($tags) && $display_filter == '1' ):?>
        <li class="active"><a href="javascript:;" data-filter="*"><?php _e( 'Все', 'cactus' );?></a></li>
